...
for index, row in df_l.iterrows():
    column_a_value = row['Emp ID']
    column_b_value = row['Date']

    # Check if the values in df1 exist in df2
    if ((grouped_jmdo['Emp ID'] == column_a_value).any() and 
        (grouped_jmdo['Date'] == column_b_value).any()):
        
        df_l['Flag A'] = df_l.apply(
            lambda x: 'Analyse' if (
                x['JMDO/JMDL']=='JMDO' and
                x['Speed of Travel (KM/Hr)'] > 75
...
for index, row in df_l.iterrows():
    ...
    if ((grouped_jmdo['Emp ID'] == column_a_value).any() and 
        ...
        df_l['Flag B'] = df_l.apply(
            lambda x: 'Analyse' if                 x['JMDO/JMDL']=='JMDO' and
                            x['Speed of Travel (KM/Hr)'] > 75
and x['Occurrence'] > 8 and x['Is last checkin to Mark-out'] == 'Mark In' and x['Distance(KM)']>10 else '', 
            axis=1
        )
...
for index, row in df_l.iterrows():
    ...
    if ((grouped_jmdo['Emp ID'] == column_a_value).any() and 
        ...
        occurrences_df_c = df_l.groupby(['Emp ID', 'Date']).apply(
            lambda x: (x['Distance(KM)'] > 10).sum()
        ...
        df_l.rename(columns={'Occurrence_x':'Occurrence'},inplace=True)

        # Step 6: Merge the occurrences back to the original DataFrame
        df_l = pd.merge(df_l, occurrences_df_c[['Emp ID', 'Date']], on=['Emp ID', 'Date'], how='left')

        df_l.columns

        # import numpy as np

        df_l['Flag C'] = np.where((df_l['JMDO/JMDL'] == 'JMDO') & (df_l['Speed of Travel (KM/Hr)'] > 75) & (df_l['Occurrence'] > 3) & (df_l['Distance(KM)'] > 10), 'Analyse', '')
