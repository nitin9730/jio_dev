...
# Create a new column in DataFrame1 based on whether Column1 values exist in DataFrame2's Column2
d_df['Car Hire chk'] = d_df['CC'].isin(expanded_df['CC'])
...
d_df['KM post Mark in/Out'] = np.where(
    ...
    d_df['Distance(KM)'].clip(upper=20),
    np.where(
        (d_df['JMDO/JMDL'] == "JMDL") & (d_df['Is last checkin to Mark-out'] == "Mark In"),
        d_df['Distance(KM)'].clip(upper=40),
        '' # Assuming you want to keep the existing value if neither condition is met
    )
)
...
d_df.to_csv('Adhoc with new changes.csv')