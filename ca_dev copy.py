...
file_path = '/Users/nitin14.patil/Documents/python_work/conveyance_analysis/July 24 JPW Data.xlsx'
...
df_with_distances = pd.concat([df_with_distances,df_ca['JMDO/JMDL']], axis=1)



d_df=df_with_distances
...
df['Distance is more than 1 Km'] = df['Distance between mean to actual'].apply(
...
# Apply the function to the DataFrame
df['KM post Speed'] = df.apply(calculate_km_post_speed, axis=1)
...
# Step 3: Group by 'Emp ID', 'Month', and 'Is last checkin to Mark-out'
# Step 4: Count occurrences where 'Distance(KM)' is less than or equal to 10 km
...
c_df_ca['u_emp_id'] = c_df_ca['TRDMFIELD1'].apply(lambda x: x[1:] if x.startswith('P') else x)
...
def check_record_match(row, df2):
    matches = df2[(df2['u_emp_id'] == row['Emp ID']) &
...
checkdf = df[df['Travel_Record_Match'] == False]
