...
checkdf= df[(df['Emp ID']==50123511) & (df['Date']=='2024-07-10')]
...
df_l['Month'] = pd.to_datetime(df_l['Date']).dt.to_period('M')
# Merge with suffixes
df_l = df_l.merge(occurrences_df, on=['Emp ID', 'Month'], how='left', suffixes=('', '_occurrences'))
...
# Rename the column if needed
occurrences_df_c.rename(columns={'Occurrences_c': 'Occurrence'}, inplace=True)


# Step 2: Extract month from 'Date' column
occurrences_df_c['Month'] = pd.to_datetime(occurrences_df_c['Date']).dt.to_period('M')


# Merge with suffixes
df_l = df_l.merge(occurrences_df_c, on=['Emp ID', 'Month'], how='left', suffixes=('', '_occurrences'))
...
# Convert columns to the appropriate types if necessary
df_l['Emp ID'] = df_l['Emp ID'].astype('int64')
c_df_ca['TRDMFIELD1'] = c_df_ca['TRDMFIELD1'].astype('int64')
...
# Merge df1 and df2 based on Emp ID and filter by date range
merged_df = pd.merge(df_l, c_df_ca, left_on='Emp ID', right_on='TRDMFIELD1', how='left')

# Check if Date in df1 is within the start and end dates in df2
df_l['Travel_Record_Match'] = (
    (merged_df['TRDMBOOKSTARTDATE'] <= merged_df['Date']) &
    (merged_df['TRDMBOOKENDDATE'] >= merged_df['Date'])
)

# If there's no match in the corresponding merged rows, Travel_Record_Match will be False
df_l['Travel_Record_Match'] = df_l['Travel_Record_Match'].fillna(False)
...
# df_l['Travel_Record_Match'] =''
# # Set the 'Travel_Record_Match' column to True or False based on the mask
# df_l['Travel_Record_Match'] = date_mask

# # If you want the result as True/False, you can convert it to boolean:
# df_l['Travel_Record_Match'] = df_l['Travel_Record_Match'].fillna(False).astype(bool)


checkdf= df_l[(df_l['Emp ID']==50123511) & (df_l['Date']=='2024-07-05')]
...
with pd.ExcelWriter('final_output_all_columns.xlsx', engine='xlsxwriter') as writer:
...
print("Export completed successfully! final_output_all_columns")
...
# Create an Excel writer object
with pd.ExcelWriter('final_output_limited_columns.xlsx', engine='xlsxwriter') as writer:
    for i in range(num_sheets):
        start_row = i * max_rows_per_sheet
        end_row = (i + 1) * max_rows_per_sheet
        
        # Slice the DataFrame for the current sheet
        chunk = df_final.iloc[start_row:end_row]
        
        # Write the chunk to a new sheet
        chunk.to_excel(writer, sheet_name=f'Sheet{i + 1}', index=False)

print("Export completed successfully! final_output_limited_columns")
